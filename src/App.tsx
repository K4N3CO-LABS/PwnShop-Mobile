...
export default function App() {
  useEffect(() => {
    ...
    // Hide static splash if it exists, with a minimum duration
    ...
    <div className="p-4 space-y-6">
      <div className={`relative rounded-3xl border ${isDarkMode ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-emerald-500/10 border-emerald-500/40'} overflow-hidden shadow-sm`}>
        ...
        <div className="relative z-10 p-6">
          <h2 className={`text-2xl font-bold mb-2 ${isDarkMode ? 'text-white' : 'text-emerald-700'}`}>Welcome to The PwnShop</h2>
          <p className={`${isDarkMode ? 'text-zinc-300' : 'text-emerald-600/80'} mb-4 font-medium`}>The absolute best (and most insecure) deals on the web.</p>
          <div className="flex space-x-2">
            <button onClick={() => changeView('about')} className={`${isDarkMode ? 'bg-white/10 text-white hover:bg-white/20' : 'bg-emerald-500/20 text-emerald-800 hover:bg-emerald-500/30'} backdrop-blur-sm px-4 py-2 rounded-xl text-sm font-bold active:scale-95 transition-all flex items-center space-x-2`}>
               <Info size={16} />
               <span>What is this?</span>
            </button>
            <button onClick={() => changeView('shop')} className="bg-emerald-500 text-white px-4 py-2 rounded-xl text-sm font-bold shadow-sm hover:opacity-90 active:scale-95 transition-all">
              Shop Now
            </button>
          </div>
        </div>
      ...
      <div onClick={() => changeView('scoreboard')} className={`relative p-5 rounded-2xl cursor-pointer active:scale-95 transition-transform shadow-sm ${isDarkMode ? 'bg-emerald-500/10 border border-emerald-500/20' : 'bg-emerald-500/10 border border-emerald-500/40'}`}>
      ...
      <div>
        ...
        <div className="flex space-x-4 overflow-x-auto pb-4 snap-x pr-4 -mx-4 px-4 pl-4 no-scrollbar">
          {PRODUCTS.slice(0, 6).map(p => (
            <div key={p.id} className={`snap-start flex-none w-48 p-5 rounded-[1.25rem] shadow-sm border flex flex-col items-center h-auto ${isDarkMode ? 'bg-emerald-500/10 border-emerald-500/20 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.5)]' : 'bg-emerald-500/10 border-emerald-500/40 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)]'} transition-transform active:scale-95`}>
              <div className="flex flex-col items-center flex-1 w-full text-center">
                <div className={`w-20 h-20 rounded-full flex items-center justify-center text-4xl mb-3 shadow-inner ${isDarkMode ? 'bg-zinc-800/80 mb-3' : 'bg-zinc-50'}`}>
                  {p.image}
                </div>
                <span className={`font-bold text-sm mb-1 ${isDarkMode ? 'text-zinc-100' : 'text-zinc-900'}`}>{p.name}</span>
                <span className={`text-[11px] mb-3 leading-relaxed flex-1 ${isDarkMode ? 'text-zinc-500' : 'text-zinc-500'}`}>{p.desc}</span>
              </div>
      ...
      <div className={`p-6 rounded-[1.5rem] shadow-sm border mt-4 ${isDarkMode ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-emerald-500/10 border-emerald-500/40 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)]'}`}>
        <h3 className={`font-bold mb-5 text-lg ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}>Community Feedback</h3>
        <div className="space-y-4 mb-6">
          {feedbacks.map((f, i) => (
             <div key={i} className={`p-4 rounded-2xl border ${isDarkMode ? 'bg-zinc-900/30 border-white/5' : 'bg-zinc-50/50 border-zinc-100'}`}>
                <div className="flex items-center space-x-2 mb-2">
                   ...
                   <p className="text-xs font-bold text-zinc-500">{f.author}</p>
                </div>
                {f.html ? <div dangerouslySetInnerHTML={{ __html: f.text }} className={`text-sm italic border-l-2 border-emerald-500/40 pl-3 py-1 ${isDarkMode ? 'text-zinc-300' : 'text-zinc-700'}`}></div> : <p className={`text-[13px] leading-relaxed ${isDarkMode ? 'text-zinc-300' : 'text-zinc-700'}`}>{f.text}</p>}
             </div>
          ))}
        </div>
        <div className="space-y-3 border-t border-white/5 pt-5">
           <input type="text" value={feedbackAuthor} onChange={e => setFeedbackAuthor(e.target.value)} className={`w-full ${isDarkMode ? 'bg-zinc-800/50 text-zinc-200 border-white/5' : 'bg-zinc-50 text-zinc-800 border-zinc-200'} border px-4 py-3 rounded-xl text-sm outline-none focus:ring-2 focus:ring-emerald-500 transition-all`} placeholder="Author Name" />
           <textarea value={feedbackInput} onChange={e => setFeedbackInput(e.target.value)} className={`w-full ${isDarkMode ? 'bg-zinc-800/50 text-zinc-200 border-white/5' : 'bg-zinc-50 text-zinc-800 border-zinc-200'} border px-4 py-3 rounded-xl text-sm outline-none focus:ring-2 focus:ring-emerald-500 min-h-[100px] transition-all`} placeholder="Write a review..."></textarea>
           <button onClick={() => {
              const xssPattern = new RegExp('<[a-z1-6]+.*?>', 'i');
  ...
  const renderAbout = () => (
    <div className="p-4 space-y-6 flex flex-col min-h-max pb-32">
      ...
      <div className="space-y-4">
        ...
        <div className={`${isDarkMode ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-emerald-500/10 border-emerald-500/40'} rounded-xl p-4 border flex items-start space-x-3 shadow-sm`}>
      ...
      <div className="p-4 space-y-5">
        <div className="relative group">
          <input 
            ...
            className={`w-full pl-12 pr-4 py-3.5 rounded-2xl border ${isDarkMode ? 'bg-emerald-500/10 border-emerald-500/20 text-white placeholder-zinc-500' : 'bg-emerald-500/10 border-emerald-500/40 text-zinc-900 placeholder-zinc-400'} focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 outline-none transition-all shadow-sm backdrop-blur-md`}
          />
          <Search className="absolute left-4 top-4 text-emerald-500/50 group-focus-within:text-emerald-500 w-5 h-5 transition-colors" />
        </div>
        
        {searchQuery && (
          <div className="text-sm text-zinc-500 bg-emerald-500/10 p-5 rounded-2xl border border-emerald-500/30 shadow-inner backdrop-blur-sm -mt-2">
            <p className="mb-2 font-bold text-zinc-300">Search Results for:</p>
            {/* EDUCATIONAL VULNERABILITY: dangerouslySetInnerHTML without sanitization allows XSS */}
            <div className="break-all italic border-l-4 border-emerald-500/40 pl-3 py-1 font-mono text-[13px]" dangerouslySetInnerHTML={{ __html: searchQuery }} />
            <p className="text-[11px] font-medium text-emerald-400 mt-3 pt-3 border-t border-emerald-500/20">Hint: What happens if you search for an invalid HTML tag like &lt;iframe&gt; or &lt;img src="x" onerror="alert(1)"&gt;?</p>
          </div>
        )}

        <div className="space-y-4">
          {PRODUCTS.filter(p => p.name.toLowerCase().includes(searchQuery.toLowerCase())).map(p => (
            <div key={p.id} className={`p-5 rounded-2xl flex flex-col shadow-sm border ${isDarkMode ? 'bg-emerald-500/10 border-emerald-500/20 hover:border-emerald-500/30' : 'bg-emerald-500/10 border-emerald-500/40 hover:border-emerald-500/30'} transition-all group`}>
              <div className="flex items-start justify-between">
                <div className="flex items-center space-x-4 mb-3">
                  <div className={`w-14 h-14 rounded-2xl flex flex-shrink-0 items-center justify-center text-3xl shadow-inner ${isDarkMode ? 'bg-zinc-800' : 'bg-zinc-100'}`}>
                    {p.image}
                  </div>
                  <div>
                    <h4 className={`font-bold text-sm ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}>{p.name}</h4>
                    <p className="text-emerald-500 font-bold text-sm leading-none mt-1">${p.price.toFixed(2)}</p>
                  </div>
                </div>
                <button 
    ...
    <div className="p-4 space-y-6">
      ...
      {cart.length === 0 ? (
        <div className={`flex flex-col items-center justify-center p-12 border-2 border-dashed text-center rounded-[1.5rem] h-64 ${isDarkMode ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-emerald-500/10 border-emerald-500/40'}`}>
           <div className={`p-5 rounded-full mb-5 shadow-inner ${isDarkMode ? 'bg-zinc-800/80 text-zinc-500' : 'bg-zinc-200 text-zinc-400'}`}>
             <ShoppingCart className="w-12 h-12" />
           </div>
           <p className={`text-lg font-bold mb-1 ${isDarkMode ? 'text-zinc-300' : 'text-zinc-800'}`}>Your cart is empty</p>
           <p className={`text-[13px] max-w-[220px] leading-relaxed ${isDarkMode ? 'text-zinc-500' : 'text-zinc-500'}`}>Looks like you haven't added anything yet. Start browsing to find something nice!</p>
        </div>
      ) : (
        <div className="space-y-4">
          {cart.map(item => {
            ...
            if (!p) return null;
            return (
              <div key={item.id} className={`p-4 rounded-2xl flex items-center justify-between shadow-sm border ${isDarkMode ? 'bg-emerald-500/10 border-emerald-500/20 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.5)]' : 'bg-emerald-500/10 border-emerald-500/40 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)]'} transition-all`}>
                <div className="flex items-center space-x-4">
                  <div className={`w-14 h-14 rounded-xl flex items-center justify-center text-3xl shadow-inner ${isDarkMode ? 'bg-zinc-900' : 'bg-zinc-50'}`}>
                    {p.image}
                  </div>
                  <div>
                    <h4 className={`font-bold text-sm ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}>{p.name}</h4>
                    <p className="text-emerald-500 font-bold mt-1 text-sm">${p.price}</p>
                  </div>
                </div>
                <div className="flex items-center space-x-3">
                  <div className={`flex items-center rounded-xl p-1 border shadow-inner ${isDarkMode ? 'bg-black border-white/5' : 'bg-emerald-500/10 border-emerald-500/40'}`}>
                    <input 
                      type="text" 
                      className={`w-12 text-center bg-transparent outline-none font-bold ${isDarkMode ? 'text-zinc-200' : 'text-zinc-800'}`} 
                      value={item.qty}
                  ...
                  <button 
                    ...
                    className={`p-3 rounded-xl transition-all shadow-sm ${isDarkMode ? 'text-zinc-500 hover:text-red-400 hover:bg-red-500/10 bg-zinc-900' : 'text-zinc-400 hover:text-red-500 hover:bg-red-500/10 bg-zinc-100'}`}
                    title="Remove item"
                  >
                    <Trash2 size={18} />
                  </button>
          ...
          <div className={`p-6 rounded-[1.5rem] mt-8 ${isDarkMode ? 'bg-[#0a0a0a] text-white border border-white/5 shadow-[inset_0_2px_20px_rgba(0,0,0,1)]' : 'bg-zinc-50 text-zinc-900 border border-zinc-200 shadow-inner'}`}>
            <div className="flex justify-between items-center mb-6">
              <span className={`font-bold uppercase tracking-widest text-[11px] ${isDarkMode ? 'text-zinc-500' : 'text-zinc-500'}`}>Total Amount</span>
              <span className={`text-3xl font-bold ${cartTotal < 0 ? 'text-emerald-400' : ''}`}>
                ${cartTotal.toFixed(2)}
              </span>
            </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            {isRegistering && (
              <div>
                ...
                <input name="username" type="text" placeholder="neo_1337" className={`w-full p-3.5 rounded-2xl border outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all text-sm ${isDarkMode ? 'bg-emerald-500/10 text-white border-emerald-500/20 shadow-inner' : 'bg-emerald-500/10 text-zinc-900 border-emerald-500/40 shadow-inner'}`} required />
              </div>
            )}
            <div>
              <label className={`block text-sm font-bold mb-1.5 flex items-center justify-between ${isDarkMode ? 'text-zinc-300' : 'text-zinc-700'}`}>
                <span>Email</span>
                {!isRegistering && <span className="text-[11px] text-zinc-500 font-normal">user@example.com</span>}
              </label>
              <input name="email" type="text" placeholder="user@example.com" className={`w-full p-3.5 rounded-2xl border outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all text-sm ${isDarkMode ? 'bg-emerald-500/10 text-white border-emerald-500/20 shadow-inner' : 'bg-emerald-500/10 text-zinc-900 border-emerald-500/40 shadow-inner'}`} required />
            </div>
            <div>
              <label className={`block text-sm font-bold mb-1.5 flex items-center justify-between ${isDarkMode ? 'text-zinc-300' : 'text-zinc-700'}`}>
                <span>Password</span>
                {!isRegistering && <span className="text-[11px] text-zinc-500 font-normal">password123</span>}
              </label>
              <input name="password" type="password" placeholder="••••••••" className={`w-full p-3.5 rounded-2xl border outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all text-sm ${isDarkMode ? 'bg-emerald-500/10 text-white border-emerald-500/20 shadow-inner' : 'bg-emerald-500/10 text-zinc-900 border-emerald-500/40 shadow-inner'}`} required />
            </div>
            
            <button type="submit" className="w-full bg-gradient-to-r from-emerald-500 to-emerald-400 text-white font-bold py-3.5 rounded-2xl shadow-[0_0_20px_rgba(16,185,129,0.3)] hover:shadow-[0_0_30px_rgba(16,185,129,0.5)] active:scale-95 transition-all mt-4 text-[15px]">
           ...
           <div className="flex flex-col items-center space-y-4">
              ...
              <div className="flex space-x-3">
                 {[1, 2, 3, 4].map(i => (
                   <div key={i} className={`w-12 h-14 rounded-xl border flex items-center justify-center text-emerald-500 font-mono text-2xl shadow-inner ${isDarkMode ? 'bg-zinc-900/50 border-white/5 shadow-[inset_0_2px_10px_rgba(0,0,0,0.5)]' : 'bg-zinc-100 border-zinc-200 shadow-[inset_0_2px_10px_rgba(0,0,0,0.05)]'} transition-all`}>
                     *
                   </div>
                 ))}
              </div>
  ...
  const renderAdmin = () => {
    if (user?.role !== 'admin' && !isAdminOverridden) {
      return (
        <div className="p-8 h-full flex flex-col items-center justify-center space-y-6 text-center">
            <div className={`p-8 rounded-[2rem] border relative overflow-hidden flex flex-col items-center w-full max-w-sm ${isDarkMode ? 'bg-red-500/10 border-red-500/20 shadow-[0_4px_30px_-5px_rgba(239,68,68,0.2)]' : 'bg-red-500/10 border-red-500/40 shadow-[0_4px_30px_-5px_rgba(239,68,68,0.15)]'}`}>
                <div className="absolute inset-0 bg-gradient-to-br from-red-500/10 to-transparent pointer-events-none" />
                <ShieldAlert className="w-16 h-16 text-red-500 mb-5 drop-shadow-md animate-pulse" />
                <h2 className={`text-xl font-black uppercase tracking-widest ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}>Access Restricted</h2>
                <p className={`text-sm mt-2 max-w-[220px] pb-4 ${isDarkMode ? 'text-zinc-500' : 'text-zinc-600'}`}>This area requires Level 4 Security Clearance or a valid Manager Override Code.</p>
                <div className="w-full absolute bottom-0 left-0 h-1 bg-gradient-to-r from-red-500 via-transparent to-red-500 opacity-50"></div>
            </div>
            ...
            <div className="w-full max-w-sm space-y-4">
                <div className="relative group">
                    <input 
                        ...
                        className={`w-full ${isDarkMode ? 'bg-[#0a0a0a] border-white/10 text-emerald-500 placeholder-zinc-700' : 'bg-emerald-500/10 border-emerald-500/40 text-emerald-600 placeholder-zinc-400'} rounded-2xl p-4 text-center font-mono focus:border-red-500/50 focus:ring-4 focus:ring-red-500/10 outline-none transition-all uppercase shadow-inner`}
                    />
                </div>
                <button 
                    onClick={() => {
                    ...
                    className="w-full bg-red-500 hover:bg-red-600 text-white font-bold py-4 rounded-2xl shadow-[0_0_20px_rgba(239,68,68,0.3)] transition-all active:scale-95"
                >
                    INITIATE OVERRIDE
                </button>
            </div>
            
            <p className="text-[10px] text-zinc-500 font-mono tracking-widest mt-8">ERR_UNAUTHORIZED_ACCESS_LOGGED</p>
        </div>
      );
    }
    ...
    return (
      ...
      <div className={`p-5 rounded-2xl flex items-start space-x-4 mb-4 ${isDarkMode ? 'bg-red-500/10 border-red-500/20 shadow-[inset_0_2px_10px_rgba(239,68,68,0.1)]' : 'bg-red-50 border-red-200 shadow-sm'}`}>
        ...
        <div>
          <h3 className={`font-black uppercase tracking-widest text-sm mb-1 ${isDarkMode ? 'text-red-400' : 'text-red-800'}`}>Administration Panel</h3>
          <p className={`text-xs ${isDarkMode ? 'text-red-500' : 'text-red-600'}`}>Strictly confidential! Do not share.</p>
        </div>
      </div>

      <div className={`p-5 rounded-2xl shadow-sm border ${isDarkMode ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-emerald-500/10 border-emerald-500/40 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)]'} space-y-4`}>
        <h4 className={`font-bold border-b pb-3 flex items-center space-x-2 ${isDarkMode ? 'text-white border-white/5' : 'text-zinc-900 border-zinc-100'}`}>
          <Globe size={16} className="text-emerald-500" />
          <span>Internal Resource Fetcher</span>
        </h4>
        <p className={`text-xs ${isDarkMode ? 'text-zinc-500' : 'text-zinc-500'}`}>Ping an external resource or internal service to check status.</p>
        <div className="flex space-x-2">
          <input type="text" id="fetchUrl" defaultValue="https://example.com/api/ping" className={`flex-1 px-4 py-2.5 rounded-[12px] text-sm border outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all shadow-inner font-mono ${isDarkMode ? 'bg-black text-white border-white/10' : 'bg-emerald-500/10 text-zinc-900 border-emerald-500/40'}`} />
          <button onClick={() => {
          ...
          }} className="bg-emerald-500 hover:bg-emerald-400 text-white px-5 py-2.5 rounded-[12px] text-sm font-bold shadow-md transition-all active:scale-95">Fetch</button>
        </div>
      </div>
      ...
      <div className={`p-5 rounded-xl border ${isDarkMode ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-emerald-500/10 border-emerald-500/40'}`}>
      ...
      <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-emerald-500/10 border-emerald-500/40'}`}>
  ...
  const renderLootBox = () => {
    return (
        <div className="p-6 space-y-6">
            ...
            <div className={`border rounded-[1.5rem] p-8 text-center relative overflow-hidden mb-6 ${isDarkMode ? 'bg-emerald-500/10 border-emerald-500/20 shadow-[0_4px_20px_-10px_rgba(0,0,0,1)]' : 'bg-emerald-500/10 border-emerald-500/40 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)]'}`}>
                 <div className="absolute top-0 inset-x-0 h-32 bg-gradient-to-b from-emerald-500/10 to-transparent pointer-events-none" />
                 <div className={`w-24 h-24 mx-auto mb-6 rounded-full flex items-center justify-center text-5xl shadow-inner relative z-10 ${isDarkMode ? 'bg-zinc-900 border border-white/5' : 'bg-zinc-50 border border-zinc-100'}`}>🎁</div>
                 <div className="mb-6 relative z-10">
                     <p className={`text-[11px] font-bold mb-1 tracking-widest uppercase ${isDarkMode ? 'text-zinc-500' : 'text-zinc-400'}`}>Available Funds</p>
                     <p className="text-3xl text-emerald-500 font-mono drop-shadow-sm">${userMoney}</p>
                 </div>
                 
                 <div className="flex items-center justify-center space-x-3 mb-8 relative z-10">
                     <button onClick={() => setLootBoxBet(prev => prev - 10)} className={`w-10 h-10 rounded-xl font-bold text-lg transition-all active:scale-90 shadow-sm ${isDarkMode ? 'bg-zinc-800 text-white hover:bg-zinc-700' : 'bg-zinc-100 text-zinc-800 hover:bg-zinc-200'}`}>-</button>
                     <input type="text" value={lootBoxBet} onChange={e => setLootBoxBet(parseInt(e.target.value) || 0)} className={`w-24 text-center py-2 rounded-xl border focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 font-mono text-lg outline-none transition-all shadow-inner ${isDarkMode ? 'bg-black text-white border-white/10' : 'bg-emerald-500/10 text-zinc-900 border-emerald-500/40'}`} />
                     <button onClick={() => setLootBoxBet(prev => prev + 10)} className={`w-10 h-10 rounded-xl font-bold text-lg transition-all active:scale-90 shadow-sm ${isDarkMode ? 'bg-zinc-800 text-white hover:bg-zinc-700' : 'bg-zinc-100 text-zinc-800 hover:bg-zinc-200'}`}>+</button>
                 </div>
                 ...
                 }} className="w-full bg-gradient-to-r from-emerald-500 to-emerald-400 hover:from-emerald-400 hover:to-emerald-300 text-white font-bold py-3.5 rounded-2xl transition-all active:scale-95 shadow-[0_0_20px_rgba(16,185,129,0.3)] hover:shadow-[0_0_30px_rgba(16,185,129,0.5)] text-[15px] relative z-10">
                     Open Box (-${lootBoxBet})
                 </button>
            </div>
            ...
            <AnimatePresence>
            {lootBoxResult && (
                <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} className={`p-5 rounded-2xl border ${lootBoxResult.isWin ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-500' : 'bg-red-500/10 border-red-500/20 text-red-400'} text-center text-[15px] font-medium shadow-sm backdrop-blur-sm`}>
        ...
        <div className={`p-6 h-full flex flex-col font-mono transition-colors ${isDarkMode ? 'bg-zinc-950 text-zinc-300' : 'bg-zinc-50 text-zinc-800'}`}>
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pb-8">
                {Object.entries(marketTools).map(([id, t]) => {
                    ...
                    return (
                        <div key={id} className={`border rounded-[1.25rem] p-5 flex items-start space-x-5 transition-all ${owned ? (isDarkMode ? 'bg-zinc-900/30 border-white/5 opacity-50' : 'bg-zinc-100/50 border-zinc-200 opacity-50') : (isDarkMode ? 'bg-red-500/10 border-red-500/20 shadow-[0_4px_20px_-10px_rgba(239,68,68,0.2)] hover:border-red-500/40' : 'bg-red-500/10 border-red-500/40 shadow-[0_4px_20px_-10px_rgba(239,68,68,0.1)] hover:border-red-500/50')}`}>
                            <div className={`text-3xl w-14 h-14 flex items-center justify-center rounded-2xl shadow-inner ${isDarkMode ? 'bg-zinc-900 border border-white/5' : 'bg-zinc-50 border border-zinc-100'}`}>
                                {t.icon}
                            </div>
                            <div className="flex-1">
                                <h3 className={`font-bold uppercase tracking-wide text-[15px] mb-1 ${owned ? 'text-zinc-500' : (isDarkMode ? 'text-red-400' : 'text-red-600')}`}>{t.name}</h3>
                                <p className={`text-[12px] mb-4 leading-relaxed ${isDarkMode ? 'text-zinc-500' : 'text-zinc-500'}`}>{t.desc}</p>
                                <button 
                                    onClick={() => buyTool(id, t.price)}
                                    disabled={owned || bountyCoins < t.price}
                                    className={`text-xs font-bold px-4 py-2.5 rounded-xl w-full transition-all shadow-sm ${owned ? (isDarkMode ? 'bg-zinc-900 text-zinc-600 cursor-not-allowed' : 'bg-zinc-200 text-zinc-500 cursor-not-allowed') : bountyCoins >= t.price ? (isDarkMode ? 'bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white active:scale-95 shadow-[0_0_15px_rgba(239,68,68,0.2)] hover:shadow-[0_0_20px_rgba(239,68,68,0.4)]' : 'bg-red-50 text-red-600 hover:bg-red-500 hover:text-white active:scale-95 shadow-[0_0_15px_rgba(239,68,68,0.1)]') : (isDarkMode ? 'bg-black text-red-900/40 border border-red-900/20 cursor-not-allowed' : 'bg-zinc-50 text-red-300 border border-red-100 cursor-not-allowed')}`}
                                >
                ...
                {inventory.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        {inventory.map(toolId => {
                            ...
                            return (
                                <div key={toolId} className={`border rounded-xl p-5 flex flex-col ${isDarkMode ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-emerald-500/10 border-emerald-500/40'}`}>
                                    <div className="flex items-center space-x-4 mb-3">
  ...
  const renderDebug = () => (
    <div className="p-4 space-y-5">
      <button 
        onClick={() => changeView('admin')}
        className={`flex items-center space-x-2 font-bold mb-2 transition-colors px-2 py-1 ${isDarkMode ? 'text-zinc-500 hover:text-zinc-300' : 'text-zinc-400 hover:text-zinc-600'}`}
      >
        <ArrowLeft size={16} />
        <span>Back to Admin</span>
      </button>

      <div className={`p-5 rounded-2xl flex items-start space-x-4 mb-6 shadow-inner border ${isDarkMode ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-emerald-50 border-emerald-100'}`}>
        <div className={`p-2 rounded-xl shadow-inner ${isDarkMode ? 'bg-emerald-500/10' : 'bg-emerald-200/50'}`}>
           <Terminal className="text-emerald-500 w-6 h-6 flex-shrink-0" />
        </div>
        <div>
          <h3 className={`font-black uppercase tracking-widest text-sm mb-1 ${isDarkMode ? 'text-emerald-400' : 'text-emerald-700'}`}>System Logs</h3>
          <p className={`text-xs ${isDarkMode ? 'text-zinc-500' : 'text-emerald-600'}`}>Hardware level debug outputs.</p>
        </div>
      </div>
      
      <div className="bg-[#000] rounded-[1.5rem] p-5 font-mono text-[11px] text-emerald-400 min-h-[300px] max-h-[40vh] overflow-y-auto space-y-1.5 shadow-[inset_0_2px_15px_rgba(0,0,0,1)] border border-emerald-500/20">
        {debugLogs.length === 0 ? <p className="opacity-50">Logs cleared.</p> : debugLogs.map((log, i) => <div key={i} className="border-l border-emerald-500/30 pl-2 opacity-90"><span className="text-zinc-600">[{new Date().toISOString().split('T')[1].slice(0, -1)}]</span> {log}</div>)}
      </div>

      <div className={`p-5 rounded-[1.5rem] border shadow-sm ${isDarkMode ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-emerald-500/10 border-emerald-500/40'} space-y-4`}>
         <div className="flex items-center justify-between border-b border-white/5 pb-3">
            <div className="flex items-center space-x-3">
               <div className={`p-2 rounded-lg shadow-inner ${isDarkMode ? 'bg-zinc-900 border border-white/5' : 'bg-zinc-50 border border-zinc-100'}`}>
                 <Cpu className="text-zinc-500 w-5 h-5" />
               </div>
               <span className={`text-[11px] font-bold uppercase tracking-widest ${isDarkMode ? 'text-zinc-400' : 'text-zinc-600'}`}>Advanced Forensic Tools</span>
            </div>
         </div>
         <button 
           onClick={() => {
    ...
    return (
      <div className="p-4">
        <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-6 text-white text-center shadow-lg mb-6 relative overflow-hidden">
          ...
          <p className="text-zinc-400 text-sm mb-6 relative z-10">Find vulnerabilities to earn Bounty Coins.</p>
          
          <div className="flex justify-between items-center mb-6 relative z-10">
            <div className="flex flex-col items-center flex-1">
              <span className="text-3xl font-bold text-emerald-400">{solvedChallenges.length}</span>
              <span className="text-xs text-zinc-500 mt-1">Solved</span>
            </div>
            <div className="w-px h-10 bg-white/10 mx-2"></div>
            <div className="flex flex-col items-center flex-1">
              <span className="text-3xl font-bold text-zinc-300">{CHALLENGES.length - solvedChallenges.length}</span>
          ...
          <div className="w-full bg-[#222] rounded-full h-2 mb-2 relative z-10">
            <div className="bg-emerald-500 h-2 rounded-full transition-all duration-1000" style={{ width: `${progressPerc}%` }}></div>
          </div>
          <p className="text-emerald-500 font-bold text-xs text-right relative z-10">{progressPerc}% Complete</p>
          
          {/* Subtle Grid Background */}
          <div className="absolute inset-0 z-0 opacity-[0.03]" style={{ backgroundImage: 'radial-gradient(circle, #fff 1px, transparent 1px)', backgroundSize: '16px 16px' }}></div>
        </div>
            ...
            return (
              <div key={level} className="space-y-3">
                ...
                {levelChallenges.map(c => {
                  ...
                  return (
                    <div key={c.id} className={`p-5 rounded-2xl border flex items-start space-x-4 transition-all ${solved ? 'bg-emerald-500/10 border-emerald-500/30' : (isDarkMode ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-emerald-500/10 border-emerald-500/40')}`}>
                      <div className={`mt-0.5 flex-shrink-0 w-4 h-4 rounded-full flex items-center justify-center ${solved ? 'text-emerald-500' : (isDarkMode ? 'bg-zinc-800' : 'bg-zinc-200')}`}>
                        {solved ? <CheckCircle2 size={16} /> : <div className="w-2.5 h-2.5 rounded-full bg-zinc-600" />}
                      </div>
                      <div className="flex-1">
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          <h4 className={`font-bold text-base ${solved ? 'text-emerald-400' : (isDarkMode ? 'text-zinc-200' : 'text-zinc-900')}`}>{c.name}</h4>
                          <span className={`text-[10px] px-2 py-0.5 rounded-md font-bold uppercase ${
                            c.diff === 'Easy' ? 'bg-[#293c5a] text-[#a0c5f5]' :
                            c.diff === 'Medium' ? 'bg-[#4a3f1d] text-[#f5c660]' :
                            'bg-[#522525] text-[#fca5a5]'
                          }`}>
                            {c.diff}
                          </span>
                          {!solved && (
                            <span className="text-[10px] px-2 py-0.5 rounded-md font-bold uppercase bg-yellow-500/20 text-yellow-500">
                              +150 BC
                            </span>
                          )}
                        </div>
                        <p className={`text-sm ${solved ? 'text-emerald-500/80' : (isDarkMode ? 'text-zinc-500' : 'text-zinc-600')} mb-3`}>
                          {c.desc}
                        </p>
                        {solved ? (
                          <button 
                            onClick={() => setSelectedExplanation(c)} 
                            className="flex items-center space-x-2 text-xs font-bold text-emerald-400 bg-emerald-500/20 hover:bg-emerald-500/30 px-3 py-2 rounded-lg active:scale-95 transition-all w-fit"
                          >
                            <Info size={14} /> <span>Read Explanation</span>
                          </button>
                        ) : (
                          <div className="mt-2">
                            {revealedHints.includes(c.id) && c.hint ? (
                               <div className={`p-3 text-xs border rounded-lg flex items-start space-x-2 relative pr-8 ${isDarkMode ? 'bg-yellow-500/10 border-yellow-500/20 text-yellow-500' : 'bg-yellow-50 border-yellow-200 text-yellow-700'}`}>
                                 <Lightbulb size={14} className="mt-0.5 flex-shrink-0" />
                                 <span className="leading-relaxed">{c.hint}</span>
                                 <button onClick={() => setRevealedHints(prev => prev.filter(id => id !== c.id))} className="absolute top-2 right-2 opacity-50 hover:opacity-100 flex-shrink-0 p-1">
                                   <X size={14} />
                                 </button>
                               </div>
                            ) : (
                               <button onClick={() => setRevealedHints(prev => [...prev, c.id])} className={`flex items-center space-x-2 px-3 py-1.5 rounded-lg border transition-colors text-xs font-semibold active:scale-95 ${isDarkMode ? 'bg-zinc-800 border-white/10 text-zinc-400 hover:bg-zinc-700' : 'bg-zinc-100 border-zinc-200 text-zinc-600 hover:bg-zinc-200'}`}>
                                  <Info size={14} /> <span>Hint</span>
                               </button>
                            )}
                          </div>
                        )}
      ...
      >
        ...
        <div className={`${isDarkMode ? 'bg-zinc-950 border-white/5' : 'bg-zinc-100 border-zinc-200'} px-3 py-2 border-b flex items-center space-x-2 z-10 sticky top-[73px]`}>
           ...
           <form className="flex-1" onSubmit={(e) => {
               ...
               } else if (new RegExp('<[a-z1-6]+.*?>', 'i').test(val) || /javascript:/i.test(val)) {
            ...
            >
              {currentView === 'home' ? renderHome() :
               ...
               <div className="p-8 text-center text-zinc-400">
                  ...
                  {(() => {
                    // Check for DOM XSS
                    if (new RegExp('<[a-z1-6]+.*?>', 'i').test(currentView) || /javascript:/i.test(currentView)) {
